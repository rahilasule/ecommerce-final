...
include_once 'errorhandler.php';
	...
	$limit = 'LIMIT ' .($page - 1) * $rows_per_page .',' .$rows_per_page;
        ...
        $str_query="SELECT shoeid, shoename, brand.brandname, price, pic FROM shoes 
			...
			mysqli_stmt_bind_result($stmt, $sid, $shoename, $brandname, $price, $pic);
			//echo $str;
			$myarray = array();
			while(mysqli_stmt_fetch($stmt)){
					$myarray[]=array('shoename' =>$shoename , 'brandname'=>$brandname, 'price'=>$price, 'pic'=>$pic);
			}
			$stmt->close();
